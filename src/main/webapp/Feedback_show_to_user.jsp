		...
		<div>
			<div>
				<%
					if(cusDetails2  != null){
						...
						for(Customer customer: cusDetails2){
							int feedID = customer.getId();
							List<Feedback> inq = dao.getData(feedID);
							for(Feedback in: inq){
				...
				<div class="baba">
					...
					<h5>Feedback ID:  <%= in.getFid() %></h5>
					<h5>Name:  <%= in.getName() %></h5>
					<h5>Email: <%= in.getEmail() %></h5>
					<h5>Feedback:  <%= in.getFeedback() %></h5>
					
					
						<div>
							<a href = "DeleteFeedbackServlet?fid=<%=in.getFid() %> ">Delete</a>
							
							<a href = "Feedback_edit.jsp?fid=<%=in.getFid() %> ">Edit</a>
